#           Currículo
 
 ## Ana Rafaela Oliveira Camargo
 
## Idade:  16 anos
## Email:  anarafaela12camargo@gmail.com


## Objetivo Profissional
 Buscando uma oportunidade como Jovem Aprendiz na área de Tecnologia da Informação (TI), Suporte Técnico ou Apoio Administrativo
 Com o objetivo de aprender na prática e evoluir profissionalmente.
 

 ## Formação Acadêmica
 **Ensino Médio**  
 *Escola SESI DARCY ZACHARIAS* — Cursando o [2 ano] (Turno: [Manhã]
 *SENAI
 

  ## Habilidades e Competências

**Idiomas:** Português (Nativo), Inglês (Intermediário)

Facilidade com novas tecnologias

Raciocínio lógico

Conhecimento na área de Tecnologia da Informação (TI) 

Organização e Responsabilidade

Bom trabalho em equipe 

Determinação para resolver problemas.


# Conhecimentos em Tecnologia e TI

**Linguagens e Ferramentas:** HTML,Lógica de Programação, Git e GitHub...
**Sistemas e Pacote Office:** Word, Excel, PowerPoint, navegação na web e noções de hardware/redes
**Sistemas Operacionais:** Windows e Linux (básico)


# Cursos e Certificações

**[Excel Básico / Programação]** — (*Plataforma: ead.sp.senai.br)

**[Nome do Curso de TI / Programação]** — (*Plataforma: ead.sp.senai.br)

**[Nome do Curso de TI / Programação]** — (*Plataforma: ead.sp.senai.br)

**[Nome do Curso de TI / Programação]** — (*Plataforma: ead.sp.senai.br)

**[Nome do Curso de TI / Programação]** — (*Plataforma: ead.sp.senai.br)

**[Nome do Curso de TI / Programação]** — (*Plataforma: ead.sp.senai.b)

**[Nome do Curso de TI / Programação]** — (*Plataforma: ead.sp.senai.br)

**[Nome do Curso de TI / Programação]** — (*Plataforma: ead.sp.senai.br)

**[Nome do Curso de TI / Programação]** — (*Plataforma: ead.sp.senai.br)
